...
/*JNIEXPORT jint JNICALL Java_nativetest_startSimulator
  (JNIEnv *env, jobject caller, jint numParams, jobjectArray stringArray) {
    int stringCount = env->GetArrayLength(stringArray);
    const char *param[stringCount];
    for (int i=0; i<stringCount; i++) {
        jstring string = (jstring) env->GetObjectArrayElement(stringArray, i);
        const char *param[i] = env->GetStringUTFChars(string, 0);
    ...
    try
    {
        ...
        bool parseCorrect = SolverParams::parseCommandLineOption(stringCount, (char **)param, *p);
        if(!parseCorrect)
        {
