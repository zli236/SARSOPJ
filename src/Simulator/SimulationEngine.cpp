...
namespace momdp
{
	...
	*streamOut<<")" << endl;
    ...
    void SimulationEngine::setup(SharedPointer<MOMDP> problem, SharedPointer<AlphaVectorPolicy> policy,
    		SolverParams * solverParams, JNITopend * jniTopend)
    {
        this->policy = policy;
        this->problem = problem;
        this->solverParams = solverParams;
        this->jniTop = jniTopend;
    }
  ...
  void SimulationEngine::performActionUnobs(belief_vector& outBelUnobs, int action, const BeliefWithState& belSt, int currObsState) const
  {
    ...
    void SimulationEngine::print_usage(const char* cmdName)
    {
    ...
    int SimulationEngine::startSimulator(int argc, char **argv, JNITopend *topend)
    {
    	cout<<"At beginning of startSimulator, params: "<<argv;
    	jniTop = topend;
        try
        {
    	...
    	cout << "\nSimulating ..." << endl;
            ...
            for (int currSim = 0; currSim < p->simNum; currSim++)
            {
                ...
                engine.setup(problem, policy, p, jniTop);
